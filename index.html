// ─────────────────────────────────────────────────────────────
//  AuditFlow SaaS — app.js
//  Modifications par rapport au projet 74SW original :
//    1. launchApp() : ajoute un check wizard avant nav('dashboard')
//    2. checkIfOrgIsConfigured() : nouvelle fonction à la fin
...
// ─── MODIFICATION launchApp ───────────────────────────────────
// Une seule ligne change par rapport à l'original :
//   AVANT : nav('dashboard');
//   APRÈS : vérification wizard → initWizard() ou nav('dashboard')
// ─────────────────────────────────────────────────────────────
async function launchApp(){
  ...
  // ── NOUVEAU : wizard ou dashboard selon l'état de configuration ──
  var configured = await checkIfOrgIsConfigured(CU.organization_id);
  if(!configured){
    initWizard();       // défini dans wizard.js
  } else {
    nav('dashboard');   // comportement original
  }
}
...
// ─── NOUVELLE FONCTION — checkIfOrgIsConfigured ──────────────
// Interroge af_org_config via getSB() (le client Supabase
// existant de db.js) pour savoir si le wizard a été complété.
// Retourne true si configuré, false sinon.
// Fail-safe : en cas d'erreur réseau ou table manquante → true
// (ne bloque jamais l'accès à l'app en production).
// ─────────────────────────────────────────────────────────────
async function checkIfOrgIsConfigured(organization_id){
  if(!organization_id){
    // Pas d'org sur ce compte (utilisateur de démo) → pas de wizard
    console.warn('[Wizard] Pas d\'organization_id sur CU — wizard ignoré.');
    return true;
  }
  try {
    var res = await getSB()
    ...
    if(res.error){
      // Table probablement absente (première mise en prod) → ne pas bloquer
      console.warn('[Wizard] checkIfOrgIsConfigured:', res.error.message);
      return true;
    }
    // null = aucune ligne = jamais configuré
    return res.data !== null && res.data.is_configured === true;

  } catch(e){
    console.warn('[Wizard] checkIfOrgIsConfigured exception:', e.message);
    return true;
  }
}
