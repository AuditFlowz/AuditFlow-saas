...
async function launchApp(){
  ...
  document.getElementById('urole').textContent=
    ...
    CU.role==='admin'?'Admin / Directeur':'Auditeur';

  // Afficher/masquer sections réservées
  ...
  // Appliquer les paramètres de personnalisation (logo, couleur…)
  if(CU.role !== 'superadmin' && typeof settingsApplyOnLoad === 'function'){
    await settingsApplyOnLoad();
  }

  // Superadmin → dashboard direct
  if(CU.role==='superadmin'){
    nav('dashboard');
    return;
  }

  // Autres → wizard ou dashboard
...
function openModal(title,body,onOk){
  ...
  document.getElementById('mok').onclick=async function(){await onOk();closeModal();};
}
...
function badge(s){var base=s.includes('|')?s.split('|')[0]:s;return `<span class="badge ${BMAP[base]||'bpl'}">${base}</span>`;}
function pbar(s){
  var pct=0;
  if(typeof s==='number') pct=s;
  else if(s.includes('|')){var parts=s.split('|');var idx=parseInt(parts[1]);pct=Math.min(100,(idx+1)*10);}
  else{pct=PRCT[s]||0;}
...
async function checkIfOrgIsConfigured(organization_id){
  ...
  try{
    var res=await getSB().from('af_org_config').select('is_configured').eq('organization_id',organization_id).maybeSingle();
